  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div ref={scrollContainerRef} className={`auto-hide-scrollbar min-h-0 flex-1 overflow-y-auto px-[20px] py-[16px] ${isScrollbarVisible ? 'scrollbar-visible' : 'scrollbar-hidden'}`}>
        {filtered.length === 0 ? (
          <div className="mx-auto flex h-full w-full min-w-[1200px] max-w-[1200px] items-center justify-center" style={{ color: '#C3C3C3' }}>暂无需求组</div>
        ) : (
          <div className="mx-auto flex w-full min-w-[1200px] max-w-[1200px] flex-col gap-[20px]">
            {filtered.map((rg, i) =>
              <div
                key={`${fadeInKey}-${rg.id}`}
                style={{
