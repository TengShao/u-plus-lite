...
### 问题 23：交互式输入卡住
**原因：** 调试过程中错误地给所有 `read` 添加了 `</dev/tty>` 重定向，在正常交互式运行时反而导致问题。
**修复：** 移除所有不必要的 `</dev/tty>`。正确的一键部署命令是 `curl ... -o /tmp/deploy.sh && bash /tmp/deploy.sh`。
**状态：** ✅ 已修复（2026-03-31）
